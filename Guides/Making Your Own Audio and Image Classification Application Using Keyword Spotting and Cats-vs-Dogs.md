...
8- Synthesizing the quantized checkpoint to a C code (requires sample input data and the YAML file)

9- Running the generated C project on the MAX78000 which includes the KAT. The code includes weights and API to load the input, run inference and unload the result 
...
Use **ai8x-training/datasets/cats_vs_dogs.py** in ai8x-training , rename according to your own image dataset and modify the applied augmentations as needed. Maintain the folder structure as described below for train and test and copy the training (90%) and test(10%) images in folders representing your classes.  
...
If there are more than two classes, the model and the model description file (YAML) may need to be updated. More changes may be needed if the image size is changed from 128x128.



**Make sure to remove the 'augmented' folder to recreate the dataset.**
...
Copy **ai8x-synthesis/scripts/quantize_catsdogs.sh,** update with the path to the location of qat_best.pth.tar checkpoint file (e.g. in log folder).
...
Run the quantization script to generate the quantized checkpoint **qat_best-q.pth.tar** in the destination path.
...
If the model architecture or number of output classes are changed, the YAML file layers need to be updated accordingly.
...
Copy **ai8x-synthesis/scripts/gen_catsdogs_max78000.sh,** update with your own model, YAML file and quantized checkpoint and target directory, project name (prefix):
...
### 9- Build, Flash and Run KAT Code on MAX78000 Platform
...
### **DETECTING LONGER AUDIO PHRASES (>1sec) WITH MULTIPLE WORDS: **

**If you are interested in detecting longer than 1-sec multi-word audio, you can still use the same model with single words and update the kws20_demo application to detect a sequence of individual words with a simple state machine. It is NOT recommended to change the model and data loader from 128x128 (1-second) to higher dimensions for longer audio phrases. It will require significant changes to and debugging of the demo application, reduces the accuracy, and increases the response delay and false detections.**

 

### Adding New Labels

The data loader expects 1-second 16KHz mono audio .wav files for training and test data. You can use different utilities or scripts to record your own 1-second keywords. You can further augment with synthesized speech using text-to-speech python libraries (pyttsx3, gTTS). Generally, you need to have at least several hundred samples for a new keyword. Once you captured the new keyword samples, follow the below instructions.

 

#### **Recording Audio and converting to 1-sec 16khz wav files**

To record your own audio, you can use the VoiceRecorder.py script in kws20_demo\Utility and say a word many times about one word per sec with different tones and speeds and different people. Then place it in an INPUT_FOLDER and use the following script to segment it to 1-sec 16khz sampled wav files ( ffmpeg package is needed):

```bash
$ python convert_segment_wav.py -i INPUT_FOLDER -o OUTPUT_folder
```

You can also use the **speech_ptch_change.py** to further augment your 1-sec wav files by changing pitch.

 

#### **Updating the KWS example with your words**

To simplify this example, we assume that the name of the dataset, data loader, and model is not changed. Otherwise, the training, quantization, evaluation, and synthesis scripts and YAML file need to be updated with the new names as described in the previous section (Modifying Cats vs Dogs).
...
Remove **ai8x-training/data/KWS/processed** if exists so that the dataloader recreates the dataset with additional labels.
...
### 4- Train, Quantize, Evaluate, and Synthesis

Follow the general procedure to train, quantize, evaluate, and synthesize to generate the KAT C code as described before. Since there is already a sample input data for KWS20, there is no need to regenerate it.
...
- Make any other changes needed to print the correct class names.
- Build, flash and run the code as described before
